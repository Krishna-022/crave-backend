	...
	public MessageOutDto updateCart(CartInDto cartInDto, MenuItem menuItem, Long restaurantId) {
		...
		if (cartInDto.getQuantity() == 0) {
			if (cartOptional.isEmpty()) {
				log.warn("event=Cart not found and qunatity recieved=0");
			...
			if (deleteCount == 0) {
				log.warn("event=Cart item not found and qunatity recieved=0");
				return new MessageOutDto(SuccessMessageConstants.CART_UPDATED);
			}
			...
			if (cartOptional.isEmpty()) {
				...
				CartItem cartItem = new CartItem(newCartId, menuItem.getId(), menuItem.getName(), menuItem.getPrice(), cartInDto.getQuantity());
				...
				if (cartItemOptional.isEmpty()) {
					cart.setItemCount(cart.getItemCount() + 1);
					cartRepository.save(cart);
					CartItem cartItem = new CartItem(cart.getId(), menuItem.getId(), menuItem.getName(), menuItem.getPrice(), cartInDto.getQuantity());
					cartItemRepository.save(cartItem);
				} else {
